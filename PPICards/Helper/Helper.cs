...
namespace PPICards.Helper
{
    public static class Helper
    {
        public static string ConvertCardNumber(this string cardnumber, int type)
        {
            ...
            if (type == 2 && !string.IsNullOrEmpty(returnvalue) && returnvalue.Length == 16) {returnvalue = returnvalue.Substring(0, 4) + " " + returnvalue.Substring(5, 4) + " " + returnvalue.Substring(8, 4) + " " + returnvalue.Substring(12, 4);}
        ...
        public static string ConvertExpiryDate(this string ExpiryDate, int type)
        {
            ...
            if (type == 2 && !string.IsNullOrEmpty(returnvalue) && returnvalue.Length == 4){returnvalue = returnvalue.Substring(0, 2) + "/" + returnvalue.Substring(2, 2);}
        ...
        public static string ConvertCVV(this string cvv, int type)
        {
            ...
            if (type == 2 && !string.IsNullOrEmpty(returnvalue)){ returnvalue = cvv;}
        ...
        public static bool IsCheckValidFromTodate(this string Fromdate, string Todate, out string errormessage, out DateTime fromdate, out DateTime todate)
        {
            ...
            if (fromdate == DateTime.MinValue) {errormessage = "Invalid from date.<br>";}
            if (fromdate.Date > DateTime.Today || todate.Date > DateTime.Today){ errormessage = "Future date can't be allowed<br>"; }
            if (todate == DateTime.MinValue) {errormessage = errormessage + "Invalid to date.<br>";}
            if (fromdate > todate && fromdate != DateTime.MinValue && todate != DateTime.MinValue){errormessage = errormessage + "From date can't be extended to the to date.<br>"; }
            ...
            if (string.IsNullOrEmpty(errormessage)){retunvalue = false;}
            return retunvalue;
        }
